...
Solver::Solver(
        double T, double L_x, double L_y, double L_z,
...
void Solver::makeStep(Grid3D &layer, const Grid3D &previous_1, const Grid3D &previous_2) const {
    // Inner nodes
    for (int i = 1; i <= N - 1; ++i) {
        for (int j = 1; j <= N - 1; ++j) {
            for (int k = 1; k <= N - 1; ++k) {
...
Grid3D *Solver::solve() {
    ...
    for (int step = 2; step <= K; ++step) {
        ...
        if ((step + 1) % 50 == 0) {
            LOG << "Step " << step + 1 << " completed" << std::endl;
        }
    }
    delete grids[(K - 1) % 3];
    delete grids[(K - 2) % 3];
...
void Solver::fillByF(Grid3D &grid, Function4D *f, int n) const {
    for (int i = 0; i <= N; ++i) {
        for (int j = 0; j <= N; ++j) {
            for (int k = 0; k <= N; ++k) {
                grid(i, j, k) = (*f)(
